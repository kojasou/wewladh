...
namespace Wewladh.Spells
{
    public class Teleport : Spell
    {
        ...
        public override void Invoke(Character c, Character target, string args)
        {
            ...
            {
                int x, y;
                string name1 = _args[0].TrimEnd(' ').Replace(' ', '_');
                string name2 = "map_" + name1;
                int.TryParse(_args[1].Replace(" ", string.Empty), out x);
                int.TryParse(_args[2].Replace(" ", string.Empty), out y);
