...
namespace Wewladh
{
    ...
    {
        private static byte[][] seedTable = new byte[][]
        {
            #region Seed 00
            new byte[] {
        ...
        public static void Transform(Parameters p, Packet msg)
        {
            for (int i = 0; i < msg.BodyData.Length; i++)
            {
                msg.BodyData[i] ^= p.PrivateKey[i % p.PrivateKey.Length];
                msg.BodyData[i] ^= seedTable[p.Seed][msg.Ordinal];
                msg.BodyData[i] ^= seedTable[p.Seed][(i / p.PrivateKey.Length) % seedTable[p.Seed].Length];
                if ((i / p.PrivateKey.Length) % seedTable[p.Seed].Length == msg.Ordinal)
                {
                    msg.BodyData[i] = (byte)(msg.BodyData[i] ^ seedTable[p.Seed][msg.Ordinal]);
                }
            }
        }
        public static void TransformDialog(ClientPacket msg)
        {
            byte x_prime = (byte)(msg.BodyData[0] - 0x2D);
            byte x = (byte)(msg.BodyData[1] ^ x_prime);
            byte y = (byte)(x + 0x72);
            byte z = (byte)(x + 0x28);
            msg.BodyData[2] ^= y;
            msg.BodyData[3] ^= (byte)((y + 1) % 256);
            msg.BodyData[4] ^= z;
            msg.BodyData[5] ^= (byte)((z + 1) % 256);
            for (int i = 0; i < msg.BodyData.Length - 6; i++)
            {
                msg.BodyData[6 + i] ^= (byte)((z + i + 2) % 256);
            }
        }
        ...
        public class Parameters
        {
            ...
            public Parameters(byte[] key, byte seed)
            {
                PrivateKey = key;
                Seed = seed;
            }
            public Parameters(string key, byte seed)
            {
                PrivateKey = Encoding.ASCII.GetBytes(key);
                Seed = seed;
            }

            public bool Matches(Parameters p)
            {
                if ((p.Seed != Seed) || (p.PrivateKey.Length != PrivateKey.Length))
                    return false;

                for (int i = 0; i < p.PrivateKey.Length; i++)
                {
                    if (p.PrivateKey[i] != PrivateKey[i])
                        return false;
                }
